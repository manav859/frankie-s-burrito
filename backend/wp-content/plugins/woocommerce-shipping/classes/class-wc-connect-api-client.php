...
abstract class WC_Connect_API_Client {
	const API_VERSION               = '5';
	const SIFT_CONFIG_TRANSIENT_KEY = 'wcshipping_sift_config';
	...
	/**
	 * Requests the available services for this site from the WooCommerce Shipping Server
	 *
	...
	/**
	 * Validates the settings for a given service with the WooCommerce Shipping Server
	 *
	 * @param $service_slug
	...
	public function validate_service_settings( $service_slug, $service_settings ) {
		...
		if ( 1 === preg_match( '/[^a-z_\-]/i', $service_slug ) ) {
			return new WP_Error( 'invalid_service_slug', __( 'Invalid WooCommerce Shipping service slug provided', 'woocommerce-shipping' ) );
		}
			...
			if ( ( $quantity > 0 ) && $product->needs_shipping() ) {
				...
				if ( ! $product->has_weight() ) {
					return new WP_Error(
						...
						// translators: %d: Product ID.
							__( 'Product ( ID: %d ) did not include a weight. Shipping rates cannot be calculated.', 'woocommerce-shipping' ),
							$product->get_id()
						),
				...
				if (
					...
					return new WP_Error(
						...
						sprintf(
						// translators: %d: Product ID.
							__( 'Product ( ID: %d ) is missing a dimension value. Shipping rates cannot be calculated.', 'woocommerce-shipping' ),
							$product->get_id()
						),
						array( 'product_id' => $product->get_id() )
					);
	...
	/**
	 * Gets shipping rates (for checkout) from the WooCommerce Shipping Server
	 *
	...
	public function get_shipping_rates( $services, $package, $custom_boxes, $predefined_boxes ) {
		...
		if ( empty( $contents ) ) {
			return new WP_Error(
				'nothing_to_ship',
				__( 'No shipping rate could be calculated. No items in the package are shippable.', 'woocommerce-shipping' )
			);
		}
	...
	public function get_sift_configuration() {
		$sift_configs = get_transient( self::SIFT_CONFIG_TRANSIENT_KEY );
		if ( false === $sift_configs ) {
			$sift_configs = $this->request( 'GET', '/payment/sift' );
			set_transient( self::SIFT_CONFIG_TRANSIENT_KEY, $sift_configs, MONTH_IN_SECONDS );
		}
	...
	/**
	 * Gets shipping rates (for labels) from the WooCommerce Shipping Server
	 *
	...
	/**
	 * Tests the connection to the WooCommerce Shipping Server
	 *
	...
	/**
	 * Sends a request to the WooCommerce Shipping Server
	 *
	 * @param $method
	 * @param $path
	 * @param $body
	...
	protected function request_body( $initial_body = array() ) {
		...
		$body['settings'] = wp_parse_args(
			...
			array(
				...
				'wcshipping_version'   => Utils::get_wcshipping_version(),
				...
				'active_services'      => $this->loader->get_active_services(),
				'disable_stats'        => WC_Connect_Jetpack::is_safe_mode() || WC_Connect_Jetpack::is_development_site(),
			)
		);
	...
	/**
	 * Generates headers for our request to the WooCommerce Shipping Server
	 *
	 * @return array|WP_Error
	 */
	...
	protected function authorization_header() {
		...
		if ( ! $token || empty( $token->secret ) ) {
			return new WP_Error(
				'missing_token',
				__( 'Unable to send request to WooCommerce Shipping server. Jetpack Token is missing', 'woocommerce-shipping' )
			);
		}
		...
		if ( false === strpos( $token->secret, '.' ) ) {
			return new WP_Error(
				'invalid_token',
				__( 'Unable to send request to WooCommerce Shipping server. Jetpack Token is malformed.', 'woocommerce-shipping' )
			);
		}
	...
	protected function request_signature( $token_key, $token_secret, $timestamp, $nonce, $time_diff ) {
		...
		if ( $local_time < time() - 600 || $local_time > time() + 300 ) {
			return new WP_Error(
				'invalid_signature',
				__( 'Unable to send request to WooCommerce Shipping server. The timestamp generated for the signature is too old.', 'woocommerce-shipping' )
			);
		}
